...
internal static class SpanExtensions
{
    ...
    public static int FindLastNotAfter<T>(this ReadOnlySpan<T> source, int offset, int end = -1) where T : IHasStart
    {
        var result = -1;
        for (int left = 0, right = end >= 0 ? end : source.Length - 1; left <= right;)
        {
            var mid = left + (right - left) / 2;
            if (source[mid].Start >= offset)
            {
    ...
    public static int FindWhereStart<T>(this ReadOnlySpan<T> source, int offset, int end = -1) where T : IHasStart
    {
        for (int left = 0, right = end >= 0 ? end : source.Length - 1; left <= right;)
        {
            var mid = left + (right - left) / 2;
