...
internal abstract class BaseExpression(BaseExpression? parent) : IHasStart, IHasEnd, IVisualProvider
{
    ...
    private BaseExpression? _parent = parent;
        ...
        while (state.CurrentToken is { } token)
        {
            ...
            switch (token)
            {
                ...
                case UnitToken unitToken:
                    (result is null ? ref result : ref result._parent) = new UnitTerminatedExpression(result, unitToken);
                    state.Increment();
                    return result;
                ...
                case TernaryOpToken questionMark:
                    switch (current)
                    {
                        ...
                        case FunctionCall:
                        case UnaryOperation:
                        case BinaryOperation { Precedence: > ((int)OperatorType.QuestionMark) >> 8 }: // QuestionMark.Precedence()
                        case BaseValueNode:
