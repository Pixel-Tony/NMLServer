...
internal sealed partial class GRF : BaseBlockStatement
{
    ...
    public GRF(ref ParsingState state, KeywordToken keyword) : base(ref state, keyword)
    {
        ...
        while (state.CurrentToken is { } token)
        {
            switch (token)
            {
                ...
                case KeywordToken { Keyword: Keyword.Param } paramToken:
    ...
    public override void ProvideFoldingRanges(in List<FoldingRange> ranges, ref readonly PositionConverter converter)
        => IFoldingRangeProvider.RangeWithInnerRanges(OpeningBracket, ClosingBracket, _parameters, in ranges, in converter);
