...
internal ref struct Lexer(StringView view, int pos = 0)
{
    public readonly List<int> LineLengths = [];
    private int _pos = pos;
    private int _lineStart = pos;
    ...
    /// <returns>The lexed token, if any; <c>null</c> otherwise.</returns>
    /// <remarks>This method should not be called again after <c>null</c> is returned.</remarks>
    public BaseToken? LexToken()
    {
        while (IsInBounds)
        {
            char c = CurrentChar;
    ...
    private BaseToken ParseOnEqualsSign()
    {
    ...
    private BaseToken TryParseOperator(OperatorType type)
    {
        if (IsAtLastChar)
        {
    ...
    private BaseToken ParseFromSlash()
    {
        if (IsAtLastChar)
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public readonly void CompleteLine() => LineLengths.Add(_pos - _lineStart);
    ...
    private BaseToken ParseIdentifier()
    {
        int start = _pos++;
        char current = '\0';
        for (; IsInBounds; ++_pos)
        ...
        switch (current)
        {
            case '/' when charsLeft == 1
                ...
                var next = NextChar;
                NML.UnitType unitType = value switch
                {
                    "m" when next is 's' => NML.UnitType.MPS,
                    "km" when next is 'h' => NML.UnitType.KMPH,
                    _ => NML.UnitType.None
                };
                if (unitType is NML.UnitType.None)
                    break;
                _pos += 2;
            ...
            case '%' when value is "snow":
                ++_pos;
                return new UnitToken(start, NML.UnitType.Snow);
        }

        if (NML.UnitLiterals.TryGetValue(value, out var unit))
            return new UnitToken(start, unit);

        if (NML.Keywords.TryGetValue(value, out var info))
            return new KeywordToken(start, _pos, info);

        var kind = NML.GetSymbolKind(value);
